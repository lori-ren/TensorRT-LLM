...
from tensorrt_llm._torch.pyexecutor.engine.runners.decoder.forward import \
    _make_single_token_context_graph_batch
from tensorrt_llm._torch.pyexecutor.engine.runners.decoder.prepare import \
    _get_context_prompt_lookahead_token
...
def _make_forward_only_engine(
    ...
    # forward runs on the family's runner; the coordinator only dispatches
    engine._runner = object.__new__(DecoderRunner)
    engine._runner.model = SimpleNamespace(
        extra_attrs={},
        model_config=SimpleNamespace(pretrained_config=SimpleNamespace(
            rope_scaling=None)))
    engine._runner.kv_cache_manager_key = ResourceManagerType.KV_CACHE_MANAGER
    engine._runner.enable_spec_decode = False
    engine._runner.is_spec_decode = False
    engine._runner.is_draft_model = False
    engine._runner.guided_decoder = None
    engine._runner.max_beam_width = 1
    engine._runner._is_encode_only = False
    engine._runner.llm_args = SimpleNamespace(mm_encoder_only=False)
    engine._runner.mapping = SimpleNamespace(
        cp_size=1,
        enable_lm_head_tp_in_adp=False,
    )
    engine._runner.runtime_draft_len = 0
    engine._runner.attn_backend = None
    engine._runner.original_max_draft_len = 0
    engine._runner.original_max_total_draft_tokens = 0
    engine._runner._spec_dec_max_total_draft_tokens = 0
    engine._runner.spec_config = None
    engine._runner.get_runtime_tokens_per_gen_step = Mock(return_value=1)
    engine._runner.iter_states = {}
    engine._runner.forward_pass_callable = None
    engine._runner._stage_in_graph_sampling = None
    engine._runner.moe_load_balancer = None
    engine._runner._is_encoder_decoder_model = Mock(return_value=False)
    engine._runner._get_draft_kv_cache_manager = Mock(return_value=None)
    engine._runner.cuda_graph_lora_manager = None
    engine._runner._force_lora_graph_for_capture = None

    semantic_attn_metadata = Mock()
    graph_attn_metadata = Mock()
    engine._runner.attn_metadata = semantic_attn_metadata
    engine._runner._set_up_attn_metadata = Mock(
        return_value=semantic_attn_metadata)
    spec_dec_mode = Mock()
    ...
    engine._runner.spec_metadata = spec_metadata
    engine._runner._set_up_spec_metadata = Mock(return_value=spec_metadata)
    prepared_inputs = {
        "prepared": True,
        "input_ids": torch.zeros(2, dtype=torch.int32),
    }
    engine._runner._prepare_inputs = Mock(return_value=(prepared_inputs, None))
    outputs = {"logits": object()}
    engine._runner._forward_step = Mock(return_value=outputs)
    engine._runner._execute_logit_post_processors = Mock()
    engine._runner.breakable_cuda_graph_runner = None
    ...
    engine._runner.cuda_graph_runner = runner
...
class SingleTokenContextGraphBatchTestCase(unittest.TestCase):
    ...
    def test_multimodal_decode_compatibility_uses_final_prompt_token(
        ...
        request.context_current_position = 2
        self.assertTrue(
            engine._runner._is_final_multimodal_context_decode_compatible(
                request))
    ...
    def test_lora_graph_variant_selection(self) -> None:
        ...
        self.assertFalse(engine._runner._use_lora_cuda_graph(batch))

        request.lora_task_id = 42
        self.assertTrue(engine._runner._use_lora_cuda_graph(batch))
    ...
    def test_forward_commits_candidate_only_on_graph_hit(self) -> None:
        ...
        with patch(
                "tensorrt_llm._torch.pyexecutor.engine.runners.decoder.forward.torch.cuda.Event",
                return_value=event):
        ...
        engine._runner._execute_logit_post_processors.assert_called_once_with(
            batch, outputs)
        self.assertEqual(engine._runner.iter_states['num_ctx_requests'], 1)
        self.assertEqual(engine._runner.iter_states['num_ctx_tokens'], 1)
        self.assertEqual(engine._runner.iter_states['num_generation_tokens'], 1)
...
class PyTorchModelEngineTestCase(unittest.TestCase):
    ...
    def test_promoted_context_uses_prompt_token_during_overlap(self) -> None:
        ...
        inputs, _ = model_engine._runner._prepare_tp_inputs(
    ...
    def test_pad_generation_requests(self) -> None:
        ...
        for (batch_size, max_seq_len) in batch_sizes_and_seqlens:
            ...
            with model_engine._runner.cuda_graph_runner.pad_batch(
            ...
            pages_before = kv_cache_manager.get_num_free_blocks()
            new_dummy_block = 1 if not model_engine._runner.cuda_graph_runner.padding_dummy_requests else 0
            with model_engine._runner.cuda_graph_runner.pad_batch(
                    batch, resource_manager) as padded_batch:
    ...
    def test_warmup(self):
        ...
        padding_dummies = model_engine._runner.cuda_graph_runner.padding_dummy_requests
    ...
    def test_warmup_skips_padding_dummy_when_padding_impossible(self):
        ...
        self.assertEqual(
            {}, model_engine._runner.cuda_graph_runner.padding_dummy_requests)
        self.assertEqual(num_free_before,
                         kv_cache_manager.get_num_free_blocks())
    ...
    def test_preallocate_padding_dummies_uses_captured_draft_lens(self):
        ...
        runner = model_engine._runner.cuda_graph_runner
    ...
    def test_prepare_tp_inputs_with_helix_parallelism(self) -> None:
        ...
        result, _ = model_engine._runner._prepare_tp_inputs(
